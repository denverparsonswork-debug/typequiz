...
import { generateTeraQuestion, calculateEffectiveness, getExplanation } from '../logic/quiz-engine';
import type { TeraQuestion } from '../logic/quiz-engine';
import { normalizeAbilityName } from '../logic/ability-engine';
import TypeBadge from './TypeBadge';
import { useAuth } from '../context/AuthContext';
import AuthModal from './AuthModal';

interface TeraQuizProps {
  onReset: () => void;
  gen: number;
}

const TeraQuiz: React.FC<TeraQuizProps> = ({ onReset, gen }) => {
  const [question, setQuestion] = useState<TeraQuestion | null>(null);
  const [streak, setStreak] = useState(0);
  const highScoreKey = `highScore_tera_specialist_gen${gen}`;
  ...
  const nextQuestion = useCallback(async () => {
    setLoading(true);
    try {
      const q = await generateTeraQuestion(gen);
      setQuestion(q);
      setExplanation(null);
      setSelectedTypes([]);
      setLoading(false);
  ...
  const handleGuess = (type: any) => {
    if (explanation || gameOver || selectedTypes.includes(type) || loading) return;

    const isCorrect = type === question!.correctAnswer;
    setSelectedTypes(prev => [...prev, type]);

    if (isCorrect) {
      const mult = calculateEffectiveness(question!.attackerType, [type], gen, question!.pokemon.activeAbility);
      setExplanation(`Correct! Tera ${type} reduces damage to ${mult}x.`);
      ...
      setLives(newLives);
      const correctMult = calculateEffectiveness(question!.attackerType, [question!.correctAnswer], gen, question!.pokemon.activeAbility);
      const wrongMult = calculateEffectiveness(question!.attackerType, [type], gen, question!.pokemon.activeAbility);
      setExplanation(`Wrong. Tera ${type} takes ${wrongMult}x damage. The optimal choice was Tera ${question!.correctAnswer} (${correctMult}x).`);
      if (newLives <= 0) {
        setGameOver(true);
      }
    }
  ...
  if (gen < 9) {
    return (
      <div className="w-full max-w-2xl mx-auto p-12 bg-gray-900 text-white rounded-xl shadow-2xl border border-gray-700 text-center space-y-6">
        <h2 className="text-3xl font-black text-blue-500 uppercase italic">Tera Unsupported</h2>
        <p className="text-gray-400 text-lg">Terastallization was introduced in <span className="text-blue-400 font-bold">Generation 9</span>. Please select Gen 9 to play this mode.</p>
  ...
  if (loading && !question) {
    return (
      <div className="flex flex-col items-center justify-center p-20 space-y-4">
        ...
        <p className="text-gray-400 animate-pulse font-bold tracking-widest uppercase">Analyzing Matchup...</p>
      </div>
    );
  }
        ...
        <>
          <div className="mb-8 text-center space-y-6">
            <div className="space-y-2">
              <h2 className="text-lg sm:text-xl text-gray-300 font-bold uppercase tracking-tight">
                Flip the Matchup! Choose the <span className="text-blue-400 font-black">Best Defensive Tera Type</span> against:
              </h2>
              <div className="flex items-center justify-center gap-4 py-2">
                <div className="px-4 py-2 bg-red-500/10 border border-red-500/20 rounded-xl">
                  <span className="text-xs font-black text-red-500 uppercase tracking-widest mr-2">Incoming Attack</span>
                  <TypeBadge type={question.attackerType} />
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            {question.options.map((type, i) => {
              ...
              let buttonClass = "p-4 rounded-2xl transition-all transform active:scale-95 w-full bg-gray-800/50 border-2 text-center group hover:bg-gray-800 flex flex-col items-center gap-2";
              ...
              return (
                ...
                >
                  <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest group-hover:text-blue-400 transition-colors">Tera Type</span>
